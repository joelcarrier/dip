...
def convert(dataset,filename):
    ...
    if ('WindowWidth' not in dataset) or ('WindowCenter' not in dataset): # can only apply LUT if these values exist
        ...
        im = PIL.Image.frombuffer(mode, size, dataset.pixel_array, "raw", mode, 0, 1) # Recommended to specify all details by http://www.pythonware.com/library/pil/handbook/image.htm
